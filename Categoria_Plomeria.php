...
$stmt = $pdo->prepare("SELECT * FROM productos WHERE categoria = 'plomeria' ORDER BY nombre");
$stmt->execute();
...
<head>
    ...
    <title>Productos para Plomería - Gorilla Tools</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">
    <link href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.10.5/font/bootstrap-icons.css" rel="stylesheet">
    <style>
        :root { --azul-navy: #003d71; --naranja-fix: #f47920; --gris-fondo: #f8f9fa; --gris-borde: #e9ecef; }
        ...
        .product-card { background: white; border-radius: 12px; padding: 15px; box-shadow: 0 2px 8px rgba(0,0,0,0.05); transition: transform 0.3s; position: relative; }
        .product-card:hover { transform: translateY(-5px); box-shadow: 0 8px 20px rgba(0,0,0,0.1); }
        .product-checkbox { position: absolute; top: 12px; right: 12px; width: 20px; height: 20px; accent-color: var(--naranja-fix); cursor: pointer; }
        ...
        .btn-cotizar-seleccion { background: var(--azul-navy); color: white; border: none; padding: 12px 30px; border-radius: 30px; font-weight: bold; cursor: pointer; }
        .btn-cotizar-seleccion:hover { background: var(--naranja-fix); }
        .btn-cotizar-todos { background: var(--naranja-fix); color: white; border: none; padding: 12px 30px; border-radius: 30px; font-weight: bold; cursor: pointer; margin-left: 10px; }
        ...
        .pagination-btn { padding: 8px 14px; border: 1px solid var(--gris-borde); background: white; border-radius: 8px; cursor: pointer; }
        .pagination-btn:hover { background: var(--azul-navy); color: white; }
        .pagination-btn.active { background: var(--azul-navy); color: white; }
        ...
        @media (max-width: 768px) { .products-grid { grid-template-columns: repeat(auto-fill, minmax(180px, 1fr)); } .selection-info { flex-direction: column; text-align: center; } }
    </style>
</head>
<body>
...
<main class="container my-4">
    <div class="breadcrumb-custom">
        <a href="index.php">Inicio</a> / <a href="#">Plomería</a>
    </div>
    <div class="category-header">
        <h1 class="category-title">Productos para Plomería <span class="product-count" id="productCount"></span></h1>
    </div>
...
<footer style="background-color: #003d71; color: white; margin-top: 60px; padding: 40px 20px 20px;">
    <div style="max-width: 1200px; margin: 0 auto;">
        <div style="display: flex; flex-wrap: wrap; justify-content: space-between; gap: 30px;">
            ...
            <div style="display: flex; gap: 15px; margin-top: 15px;"><a href="https://www.facebook.com" target="_blank" rel="noopener noreferrer" style="color: white; font-size: 20px;"><i class="bi bi-facebook"></i></a><a href="https://www.instagram.com" target="_blank" rel="noopener noreferrer" style="color: white; font-size: 20px;"><i class="bi bi-instagram"></i></a><a href="https://web.whatsapp.com" target="_blank" rel="noopener noreferrer" style="color: white; font-size: 20px;"><i class="bi bi-whatsapp"></i></a><a href="https://www.tiktok.com" target="_blank" rel="noopener noreferrer" style="color: white; font-size: 20px;"><i class="bi bi-tiktok"></i></a></div>
        </div>
        ...
        <div style="display: flex; justify-content: space-between; flex-wrap: wrap; font-size: 12px; opacity: 0.7;"><p>&copy; 2026 Gorilla Tools - Todos los derechos reservados</p><div><a href="aviso_privacidad.php" style="color: white; text-decoration: none; margin-left: 20px;">Aviso de privacidad</a><a href="terminos_y_condiciones.php" style="color: white; text-decoration: none; margin-left: 20px;">Términos y condiciones</a></div></div>
    </div>
</footer>
...
$stmt = $pdo->prepare("SELECT * FROM productos WHERE categoria = 'generadores' ORDER BY nombre");
$stmt->execute();
$productos = $stmt->fetchAll(PDO::FETCH_ASSOC);
?>
<!DOCTYPE html>
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Generadores y Bombas - Gorilla Tools</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">
    <link href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.10.5/font/bootstrap-icons.css" rel="stylesheet">
    <style>
        :root { --azul-navy: #003d71; --naranja-fix: #f47920; --gris-fondo: #f8f9fa; --gris-borde: #e9ecef; }
...
<main class="container my-4">
    <div class="breadcrumb-custom">
        <a href="index.php">Inicio</a> / <a href="#">Generadores y Bombas</a>
    </div>
    <div class="category-header">
        <h1 class="category-title">Generadores y Bombas <span class="product-count" id="productCount"></span></h1>
    </div>
    <div class="filters-bar">
        <div>
            <label>Mostrar:</label>
...
function obtenerFiltrados() {
    let filtered = productos.filter(p => !searchTerm || p.nombre.toLowerCase().includes(searchTerm) || p.clave.toLowerCase().includes(searchTerm) || p.codigo.includes(searchTerm));
    if (currentSort === 'precio-asc') filtered.sort((a, b) => a.precio - b.precio);
    if (currentSort === 'precio-desc') filtered.sort((a, b) => b.precio - a.precio);
    if (currentSort === 'nombre') filtered.sort((a, b) => a.nombre.localeCompare(b.nombre));
    return filtered;
}

function cotizar(productosList) {
    if (productosList.length === 0) {
        notificar('No hay productos seleccionados', 'error');
        return;
    }
...
function renderizar() {
    ...
    document.getElementById('productCount').innerHTML = `(${filtrados.length} productos)`;
    const start = (currentPage - 1) * itemsPerPage;
    const paginados = filtrados.slice(start, start + itemsPerPage);
    const grid = document.getElementById('productsGrid');
    
    if (paginados.length === 0) {
        grid.innerHTML = '<div class="text-center py-5"><h3>No se encontraron productos</h3></div>';
        return;
    }
    
    grid.innerHTML = paginados.map(p => `
        <div class="product-card">
            <input type="checkbox" class="product-checkbox" id="chk_${p.id}" ${seleccionados.has(p.id) ? 'checked' : ''} onchange="toggleSeleccion(${p.id})">
            <div class="product-brand">${p.marca}</div>
            <div class="product-name">${p.nombre}</div>
            <div class="product-details">Clave: ${p.clave} | Código: ${p.codigo}</div>
    ...
    let html = `<button class="pagination-btn ${currentPage === 1 ? 'disabled' : ''}" onclick="cambiarPagina(${currentPage - 1})" ${currentPage === 1 ? 'disabled' : ''}>« Anterior</button>`;
    ...
    html += `<button class="pagination-btn ${currentPage === totalPages ? 'disabled' : ''}" onclick="cambiarPagina(${currentPage + 1})" ${currentPage === totalPages ? 'disabled' : ''}>Siguiente »</button>`;
    pagDiv.innerHTML = html;
}

function cambiarPagina(p) {
    const total = Math.ceil(obtenerFiltrados().length / itemsPerPage);
    if (p >= 1 && p <= total) {
        currentPage = p;
        renderizar();
...
document.getElementById('btnBuscar')?.addEventListener('click', () => {
    searchTerm = document.getElementById('buscador').value;
    currentPage = 1;
    renderizar();
});
...
document.getElementById('cotizarSeleccionadosBtn')?.addEventListener('click', cotizarSeleccionados);
